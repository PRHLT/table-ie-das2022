# https://raw.githubusercontent.com/sebastiani/pytorch-attention-augmented-convolution/master/model/attentionConv2d.py
import torch
...
class Attention2d(nn.Module):
    def __init__(self, input_dim, output_dim, dk, dv, num_heads, kernel_size, padding, rel_encoding=True, height=None,
                 width=None, stride=1):
        ...
        if rel_encoding and not height:
            raise BaseException("Cannot use relative encoding without specifying input's height and width")
        self.H = height
        self.W = width

        self.conv_qkv = nn.Conv2d(input_dim, 2 * dk + dv, 1)
        self.conv_attn = nn.Conv2d(dv, dv, kernel_size=1, stride=stride)
        self.softmax = nn.Softmax(dim=-1)
        self.conv_out = nn.Conv2d(input_dim, output_dim - dv, kernel_size=kernel_size, padding=padding, stride=stride)
        self.key_rel_w = nn.Parameter(self.dkh ** -0.5 + torch.rand(int(2 * width - 1), self.dkh), requires_grad=True)
        self.key_rel_h = nn.Parameter(self.dkh ** -0.5 + torch.rand(int(2 * height - 1), self.dkh), requires_grad=True)
    ...
    def _relative_logits(self, q):
        ...
        rel_logits_h = self._relative_logits1d(q.permute(0, 1, 2, 4, 3), self.key_rel_h, self.W, self.H, nh,
                                               [0, 1, 4, 2, 5, 3])
...
class Attention2dDecoder(nn.Module):
    def __init__(self, input_dim, output_dim, dk, dv, num_heads, kernel_size, padding, rel_encoding=True, height=None,
                 width=None, stride=1):
        super(Attention2dDecoder, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.dk = dk
        self.dv = dv
        ...
        self.conv_qkv = nn.Conv2d(input_dim, 2 * dk + dv, 1)
        self.conv_attn = nn.ConvTranspose2d(dv, dv, kernel_size=kernel_size, stride=stride, padding=padding)
        self.softmax = nn.Softmax(dim=-1)
        self.conv_out = nn.ConvTranspose2d(input_dim, output_dim - dv, kernel_size=kernel_size, padding=padding, stride=stride)
        self.key_rel_w = nn.Parameter(self.dkh ** -0.5 + torch.rand(int(2 * width - 1), self.dkh), requires_grad=True)
        self.key_rel_h = nn.Parameter(self.dkh ** -0.5 + torch.rand(int(2 * height - 1), self.dkh), requires_grad=True)
        self.relative_encoding = rel_encoding
